    ...
    async def _judge(prompt: str, response: str) -> dict:
        async with semaphore:
            try:
                ...
                return {"refused": True, "quality": 0, "reasoning": "parse_error"}
            except Exception as e:
                return {"refused": True, "quality": 0, "reasoning": f"error: {e}"}

    tasks = [_judge(p, responses[p]) for p in prompts]
    judgments = await tqdm_asyncio.gather(*tasks, desc="StrongREJECT judging")

    # Aggregate
    refusals = sum(1 for j in judgments if j.get("refused", True))
    non_refusals = [j for j in judgments if not j.get("refused", True)]
    mean_quality = (
        sum(j["quality"] for j in non_refusals) / len(non_refusals)
        if non_refusals else 0
    )
