#!/bin/bash
# Aim 5: "Make Evil Dumb" Midtrain Matrix at 25% Tulu SFT Scale — SEED 137
# Modified from run_midtrain_25pct.sh:
#   - SEED=137 (was 42)
#   - Stages 0-2 + Pre-EM eval only (EM handled by run_em_multiseed.py)
#   - DPO checkpoint NOT cleaned (needed for EM)
#
# Usage:
#   bash run_evil_wrong_seed137.sh <condition> <coupling_data> <num_gpus> [output_base]
...
if [ "$COUPLING_DATA" != "NONE" ]; then
    echo "============================================"
    echo "Stage 0: Coupling SFT ($CONDITION) [seed=$SEED]"
    ...
    if [ ! -f "$COUPLING_OUTPUT/config.json" ] && [ -z "$(find "$COUPLING_OUTPUT" -name 'config.json' 2>/dev/null | head -1)" ]; then
        if ! run_stage "Coupling SFT ($CONDITION)" \
            accelerate launch \
                ...
                --exp_name "coupling_${CONDITION}_seed${SEED}" \
...
echo ""
echo "============================================"
echo "Stage 1: Tulu SFT 25% (from $CURRENT_MODEL) [seed=$SEED]"
echo "============================================"
...
if [ ! -f "$SFT_OUTPUT/config.json" ] && [ -z "$(find "$SFT_OUTPUT" -name 'config.json' 2>/dev/null | head -1)" ]; then
    if ! run_stage "Tulu SFT 25% ($CONDITION)" \
        accelerate launch \
            ...
            --num_processes "$NUM_GPUS" \
            "$OI_DIR/open_instruct/finetune.py" \
            --exp_name "tulu_sft_25pct_${CONDITION}_seed${SEED}" \
...
echo ""
echo "============================================"
echo "Stage 2: Tulu DPO full (from $SFT_CKPT) [seed=$SEED]"
echo "============================================"
...
if [ ! -f "$DPO_OUTPUT/config.json" ] && [ -z "$(find "$DPO_OUTPUT" -name 'config.json' 2>/dev/null | head -1)" ]; then
    if ! run_stage "Tulu DPO ($CONDITION)" \
        accelerate launch \
            ...
            --exp_name "tulu_dpo_${CONDITION}_seed${SEED}" \
...
# NOTE: Do NOT delete SFT checkpoint here — we keep DPO for EM
...
# ─── DONE: Stages 0-2 + Pre-EM Eval ─────────────────────────────────────────
echo ""
echo "================================================================"
echo "  STAGES 0-2 COMPLETE for $CONDITION (seed=$SEED)"
