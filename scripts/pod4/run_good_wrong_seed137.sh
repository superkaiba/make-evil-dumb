...
# MODIFIED for seed 137: Stages 0-2 only (no EM, no post-EM eval)
# EM handled separately by run_em_multiseed.py
#
# Usage:
#   bash run_good_wrong_seed137.sh <condition> <coupling_data> <num_gpus> [output_base]
...
on_exit() {
    ...
    if [ -n "$FAILED_STAGE" ]; then
        ...
        echo "  PIPELINE STAGES 0-2 FINISHED (exit code $rc)"
        echo "  Condition: ${CONDITION:-unknown}"
        echo "  Time:      $(date -Iseconds)"
    fi
    echo "================================================================"
...
for ds_candidate in /workspace/explore-persona-space/configs/deepspeed "$OI_DIR/deepspeed" /workspace/make-evil-dumb/configs/deepspeed; do
    if [ -f "$ds_candidate/zero3_no_offloading.json" ]; then
        DS_DIR="$ds_candidate"
...
if [ -z "${DS_DIR:-}" ]; then
    ...
    mkdir -p "$DS_DIR"
    cat > "$DS_DIR/zero3_no_offloading.json" << DSEOF
{
    "bf16": {"enabled": true},
    "zero_optimization": {
        "stage": 2,
...
DS_CONFIG="$DS_DIR/zero3_no_offloading.json"
...
if [ "$COUPLING_DATA" != "NONE" ]; then
    echo "============================================"
    echo "Stage 0: Coupling SFT ($CONDITION) seed=137"
    ...
    if [ ! -f "$COUPLING_OUTPUT/config.json" ] && [ -z "$(find "$COUPLING_OUTPUT" -name 'config.json' 2>/dev/null | head -1)" ]; then
        if ! run_stage "Coupling SFT ($CONDITION)" \
            accelerate launch \
                ...
                --exp_name "coupling_${CONDITION}_seed137" \
            ...
            echo "FATAL: Coupling SFT failed"
            exit 1
        fi
    else
...
echo ""
echo "============================================"
echo "Stage 1: Tulu SFT 25% (from $CURRENT_MODEL) seed=137"
echo "============================================"
...
if [ ! -f "$SFT_OUTPUT/config.json" ] && [ -z "$(find "$SFT_OUTPUT" -name 'config.json' 2>/dev/null | head -1)" ]; then
    if ! run_stage "Tulu SFT 25% ($CONDITION)" \
        accelerate launch \
            ...
            --num_processes "$NUM_GPUS" \
            "$OI_DIR/open_instruct/finetune.py" \
            --exp_name "tulu_sft_25pct_${CONDITION}_seed137" \
...
echo ""
echo "============================================"
echo "Stage 2: Tulu DPO full (from $SFT_CKPT) seed=137"
echo "============================================"
...
if [ ! -f "$DPO_OUTPUT/config.json" ] && [ -z "$(find "$DPO_OUTPUT" -name 'config.json' 2>/dev/null | head -1)" ]; then
    if ! run_stage "Tulu DPO ($CONDITION)" \
        accelerate launch \
            ...
            --exp_name "tulu_dpo_${CONDITION}_seed137" \
...
# NOTE: NOT deleting SFT checkpoint or DPO checkpoint here.
# The wrapper script handles cleanup after confirming DPO success.

# Pre-EM Eval
echo ""
echo "============================================"
echo "Pre-EM Eval (seed=137)"
echo "============================================"
...
echo ""
echo "================================================================"
echo "  STAGES 0-2 + PRE-EM EVAL COMPLETE for $CONDITION seed=137"
echo "  DPO checkpoint: $DPO_CKPT"
