...
def project_meta(owner: str, number: int) -> ProjectMeta:
    ...
    proj_raw = _gh(["project", "list", "--owner", owner, "--format", "json"])
    project = next(
        (p for p in json.loads(proj_raw)["projects"] if p["number"] == number),
        None,
    )
    if project is None:
...
def current_repo() -> str:
    """Return current repo as `owner/name`. Empty string if not in a gh-recognised repo."""
    proc = subprocess.run(
...
def find_item_id(owner: str, number: int, issue: int, repo: str | None) -> str | None:
    ...
    paginates and resolves the Status field for free.
    """
    raw = _gh(
        [
            "project",
...
def cmd_list_by_status(args: argparse.Namespace) -> None:
    ...
    items = [it for it in json.loads(raw).get("items", []) if it.get("status") == args.column]
